    ...
    fn draw_and_parse(&mut self, ui: &mut egui::Ui, id: egui::Id) {
        if let FileWidgetState::Finished { path, value: Ok(gui_npy_arr) } = self.test_tensor_widget.state() {
            ...
            while let Some(extent) = extents.next() {
                let mut axis_widget = InputTensorAxisWidget::default();
                axis_widget.size_widget.mode = AxisSizeMode::Fixed;
                axis_widget.size_widget.staging_fixed_size.raw = *extent;
                axis_widget.scale_widget.raw = 1.0;
